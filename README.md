...
如果您已安装 Homebrew，由于这是 Topoo Gateway 的分支版本，目前未发布到官方 Tap。请直接从 [GitHub Releases](https://github.com/viosson-d/P16-Topoo-Gateway/releases) 下载 `.dmg` 安装包。
...
# 忘记密钥？执行 docker logs topoo-gateway 或 grep -E '"api_key"|"admin_password"' ~/.topoo_gateway/gui_config.json
```
...
3. **配置文件 (持久化)**：直接修改 `~/.topoo_gateway/gui_config.json`，在 `proxy` 对象中修改或添加 `"admin_password": "您的新密码"` 字段。
