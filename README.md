...
<div align="center">
  ...
  <p>
    <img src="https://img.shields.io/badge/Version-0.0.132-blue?style=flat-square" alt="Version">
...
- Enable Tauri updater artifact generation via `createUpdaterArtifacts`
- Update the release workflow to detect `latest.json` and produce a non-empty `updater.json`

### v0.0.131 (2026-03-07)

- Fix the release workflow so macOS asset renaming no longer fails when a `.sig` file is absent
- Ready for a corrected signed updater/release run via `v0.0.131`
...
- Fixed the GitHub Release workflow to build separate macOS architecture targets instead of the invalid universal Rust target
- Ready for a new signed updater/release run via `v0.0.128`
